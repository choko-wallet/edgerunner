  ...
  return <div className='flex w-full py-1 px-2'>
    <div className="w-full border border-gray-200 rounded-lg shadow bg-gray-800 border-gray-700 h-54 p-4">
      ...
      <div className="grid grid-col-12">
        ...
        <h5 className="mb-1 text-sm font-medium text-slate-200">{`Time To Go: ${dayjs(startTime).fromNow()}`}</h5>
        <h5 className="mb-1 text-sm font-medium text-slate-400">
          <span className='text-slate-400'>{dayjs(startTime).format("DD/MM/YYYY - HH:MM")}</span>
          <span className='text-slate-300'>{`  TO `}</span>
          <span className='text-slate-400'>{dayjs(endTime).format("DD/MM/YYYY - HH:MM")}</span>
        </h5>
